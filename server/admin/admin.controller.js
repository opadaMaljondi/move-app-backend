      ...
      await admin.save(async (error, admin) => {
        if (error) {
          return res.status(200).json({
            status: false,
            error: error.message || "admin creation error",
          });
        } else {
...
exports.updateImage = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, admin profile image updated!", admin });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, error: error.message || "Internal Server Error" });
  }
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    var mailOptions = {
      from: config.EMAIL,
      to: req.body.email,
      subject: "Sending Email from Mova for Password Security",
      html: tab,
    };
