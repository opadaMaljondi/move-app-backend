...
exports.update = async (req, res) => {
  try {
    ...
    const genre_ = await Genre.find({
      name: req?.query?.name?.toUpperCase().trim(),
    });
...
exports.get = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "Successful!!", genre });
  } catch (error) {
    return res.status(500).json({
      status: false,
      error: error.message || "Internal Server error!!",
